...
def fitDougnut2D(y, fitInfo, param, weights=1):
    """
    ...
    fitresult = least_squares(fitfunDoughnut2D, fitparamStart, args=(fixedparam, fitInfo, y, weights), bounds=(lowerBounds, upperBounds))
    fitresult.fun = np.reshape(fitresult.fun, (np.shape(y)[0], np.shape(y)[1]))
    
    return fitresult
    

def fitfunDoughnut2D(fitparamStart, fixedparam, fitInfo, y, weights=1):
    """
...
def doughnut2D(Nx, Ny, x0, y0, A, sigma, offset):
    ...
    A           Amplitude
    sigma       Standard deviation of the doughnut function
    offset      DC component
